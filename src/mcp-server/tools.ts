...
// Hand-written, NOT generated by Speakeasy — augments every list operation
// with $filter and cursor query params. Safe across regen: Speakeasy never
// touches files it didn't generate.
import { augmentListTool } from "./_filter-cursor-augment.js";
...
export function createRegisterTool(
  ...
  return <A extends ZodRawShape | undefined>(tool: ToolDefinition<A>): void => {
    // Inject `filter` + `cursor` args + URL append + next_cursor wrapping
    // for any operation whose description carries the ServiceM8 annotation
    // "supports result filtering". Pass-through for everything else.
    tool = augmentListTool(tool) as ToolDefinition<A>;
